...
void init_gpio()
{
  ...
  pinMode(CHANNEL4,0);
    ...
    digitalWrite(4,HIGH);//HIGH 不使能步进电机
    digitalWrite(8,HIGH);
  ...
  */
  ...
  if(channel2_value - 1440 < -100)//back
  {
    ...
    down_pressed = true;
     ...
     if(channel2_value - 1440 > 100)//forward
    {
      //process_rf_command(1);
      up_pressed = true;
      }
       if(channel4_value - 1380 < -100)//left
      {
        //process_rf_command(3);
        left_pressed = true;
        }
         if(channel4_value - 1380 >200)//right
      {
...
//    double speed = 0.0;
//    unsigned int count = speed/(3.14*0.067/6400.0);
    ...
    /*
      ...
      */
      //counter_timeout();
        ...
        if(down_pressed)//back
  {
    process_rf_command(2);

    }
     if(up_pressed)//forward
    {
      process_rf_command(1);
